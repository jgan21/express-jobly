...
describe("POST /companies", function () {
  ...
  test("unauthorized for normal user", async function(){
    const resp = await request(app)
        .post("/companies")
        .send(newCompany)
  ...
  test("unauthorized user sending invalid data", async function(){
    const resp = await request(app)
        .post("/companies")
        .send({
          ...newCompany,
...
describe("PATCH /companies/:handle", function () {
  ...
  test("unauth user sending bad request", async function() {
    const resp = await request(app)
        .patch(`/companies/c1`)
        .send({
          logoUrl: "not-a-url",
...
describe("DELETE /companies/:handle", function () {
  ...
  test("unauth user for no such company", async function(){
    const resp = await request(app)
        .delete(`/companies/nope`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
